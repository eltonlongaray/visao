...
// ═══════════════════════════════════════════════════════════════
// ─── ÍNDICE ──────────────────────────────────────────────────
// Arquivo único: Banco de mensagens motivacionais da manhã com modal de exibição e auto-save
// ─────────────────────────────────────────────────────────────
...
// ═══════════════════════════════════════════════════════════════
// MENSAGENS — conteúdo via i18n (mm.N.title / mm.N.body)
// ═══════════════════════════════════════════════════════════════
function getMessages() {
  return [
    { n: 1, hasNotes: false, title: t('mm.1.title'), body: t('mm.1.body') },
    { n: 2, hasNotes: false, title: t('mm.2.title'), body: t('mm.2.body') },
    { n: 3, hasNotes: true,  title: t('mm.3.title'), body: t('mm.3.body'), notesPlaceholder: t('mm.3.placeholder') },
  ];
}
...
export function openMorningMessages() {
  ...
  overlay.innerHTML = `
    <div class="modal morning-msgs-modal">
      <div class="mm-header">
        <div class="mm-eyebrow">${t('mm.eyebrow')}</div>
        <div class="mm-title">${t('mm.title')}</div>
      </div>
      ...
      <div class="mm-list">
        ${messages.map(m => `
          <button type="button" class="mm-item" data-mm="${m.n}">
            ...
            <span class="mm-label">${t('mm.item.label', { n: m.n })}</span>
      ...
      <div class="modal-actions">
        <button class="btn-secondary" id="mm-close">${t('mm.close')}</button>
      </div>
    </div>
  `;
...
async function openMessageDetail(n) {
  ...
  const nextLabel = isLast ? t('mm.next.last') : t('mm.next');
  ...
  const notesBlockHtml = msg.hasNotes ? `
    <div class="mm-notes-wrap">
      <button type="button" class="mm-notes-toggle" id="mm-notes-toggle">
        ...
        <span class="mm-notes-toggle-text">${t('mm.notes.label')}</span>
  ...
  overlay.innerHTML = `
    <div class="modal morning-msgs-modal mm-detail">
      ...
      <div class="modal-actions mm-detail-actions">
        ${!isFirst ? `<button class="btn-secondary" id="mm-prev">${t('mm.prev')}</button>` : ''}
